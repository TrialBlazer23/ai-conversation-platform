...
from database import db, Conversation as ConversationModel, Message as MessageModel, ModelConfig as ModelConfigModel
...
class ConversationManager:
    ...
    def add_message(self, conversation_id: str, role: str, content: str, 
                   model_name: Optional[str] = None, tokens_used: int = 0,
                   cost: float = 0.0, metadata: Optional[Dict] = None) -> bool:
        """
    ...
    def list_conversations(self, limit: int = 50, offset: int = 0) -> List[Dict]:
        ...
        """
        conversations = db.session.query(ConversationModel)\
        ...
        return [
            {
                'id': conv.id,
                'initial_prompt': conv.initial_prompt[:100] + '...' if len(conv.initial_prompt) > 100 else conv.initial_prompt,
    ...
    def get_token_usage(self, conversation_id: str) -> Dict:
        ...
        current_config = conversation.model_configs[conversation.current_model_idx] if conversation.model_configs else None
